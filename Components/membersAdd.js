module.exports = addGrp = async (client,msg,t,gspread)=>{
    ...
    if(chat.isGroup){
        ...
        if(t['url']!=undefined){
            ...
            for (let i =1;i<end;i++){
                try{
                    console.log(`${sheet.getCell(i, 0).value} aka ${sheet.getCell(i, 1).value} was added`);
                    await chat.addParticipants([`${sheet.getCell(i, 1).value}@c.us`])
                }
                catch(e){
                    console.log(`${sheet.getCell(i, 0).value} aka ${sheet.getCell(i, 1).value} error`);
                }
            }
        }
        ...
        await msg.reply(`command can only be used in groups`)
    }         
