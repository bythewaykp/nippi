    ...
    for ( let i of db){
        ...
        if(i.id==chat.id._serialized){
            ...
            if(t['a']){
                ...
                await axios.patch(`http://localhost:3000/notes/${chat.id._serialized}`,{
                ...
                if(Object.keys(newdata).length==1){
                    await axios.delete(`http://localhost:3000/notes/${chat.id._serialized}`)
                    .then(async r=>{
...
${Object.keys(i.data).join('\n')}`)}
    ...
    if(!match){
        ...
        if(t['a']){
            ...
            await msg.reply(
`_no notes saved in group : '${chat.name}_'`
            )
        }
